...
Each manifest node names the paper statement it formalizes as
    ...
    source: ucplanar.tex:<a>-<b> (label <lab>[, ...])
...
Nodes whose `source:` names no paper label  are skipped.
"""
...
def _paper_path() -> Path:
    """The paper: the copy pinned in this repository, or `$UCPLANAR_PAPER`."""
    env = os.environ.get("UCPLANAR_PAPER")
    return Path(env) if env else ROOT / "paper" / "ucplanar.tex"


PAPER = _paper_path()

SOURCE_RE = re.compile(r"^ucplanar\.tex:(?P<a>\d+)-(?P<b>\d+)\s*(?P<rest>.*)$")
...
ANCHORABLE = {
    "theorem", "lemma", "prop", "proposition", "cor", "corollary", "definition", "problem",
