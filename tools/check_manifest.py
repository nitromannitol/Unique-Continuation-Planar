#!/usr/bin/env python3
"""Frozen-statement manifest checker for the UCPlanar formalization.
...
Verifies, against ledger/manifest.yaml:
  ...
  4. every .lean file under UCPlanar/Frozen/ is owned by exactly one node;
  5. `sorry` occurs in the production tree (UCPlanar/, UCPlanar.lean) only inside the
  ...
  8. when a node has `closure_sha256`, its source-level UCPlanar dependency-closure
...
`UCPlanar/**/*.lean` outside `UCPlanar/Frozen/`.  It scans comment- and string-masked
...
manifest node protect those bytes, while this closure covers the unfrozen UCPlanar
support declarations.

This is conservative but not an elaboration-equivalent dependency oracle.
False positives can arise when a local binder shadows a uniquely named UCPlanar
...
can also make the heuristic declaration span incomplete.  Imported non-UCPlanar
...
class SourceDecl:
    """One explicitly named source declaration outside UCPlanar/Frozen/."""
...
def current_namespace(scopes: list[tuple[str, str]]) -> str:
    ...
    for kind, name in scopes:
        if kind != "namespace":
            continue
        if name.startswith("UCPlanar.") or name == "UCPlanar":
            parts = name.split(".")
        else:
...
def source_declarations() -> list[SourceDecl]:
    """Index explicit UCPlanar declarations outside the frozen tree."""
    declarations: list[SourceDecl] = []
    library_root = ROOT / "UCPlanar"
    frozen_root = library_root / "Frozen"
    for path in sorted(library_root.rglob("*.lean")):
        ...
        for line_no, line in enumerate(lines):
            ...
            if decl_match:
                ...
                if raw_name.startswith("UCPlanar.") or raw_name == "UCPlanar":
                    full_name = raw_name
                elif namespace:
                    full_name = f"{namespace}.{raw_name}"
                else:
                    full_name = raw_name
                if not full_name.startswith("UCPlanar."):
                    continue
...
class DeclarationResolver:
    ...
    def closure(self, source: str) -> set[SourceDecl]:
        """Return the transitive explicit UCPlanar declaration closure of source."""
...
def main() -> int:
    ...
    frozen_dir = ROOT / "UCPlanar" / "Frozen"
    ...
    prod = [ROOT / "UCPlanar.lean"] + sorted((ROOT / "UCPlanar").rglob("*.lean"))
    for f in prod:
        if not f.exists():
            continue
    ...
    if args.emit:
        ...
        print(
            ...
            f"from {len(declarations)} indexed UCPlanar declarations")
        return 0
    ...
    print(
        ...
        f"{len(declarations)} UCPlanar declarations indexed)")
    return 0
