...
The manifest records whether each declaration is a draft or has a proof.
This gate checks the transitive axiom closure against that recorded state.
...
for b in man.split("  - id: ")[1:]:
    nid = b.split("\n")[0].strip()
    states[nid] = re.search(r"state: (\S+)", b).group(1)
    nodes.append((nid,
...
if r.returncode:
    ...
    sys.exit("check_axioms: Lean probe failed")

axioms = dict(re.findall(r"'([^']+)' depends on axioms: \[([^\]]*)\]", r.stdout))
for name in re.findall(r"'([^']+)' does not depend on any axioms", r.stdout):
    axioms[name] = ""
clean, dirty, missing = [], [], []
unexpected = []
...
for nid, _, exp in nodes:
    ax = axioms.get(exp)
    if ax is not None:
        extra = {a.strip() for a in ax.split(",") if a.strip()} - allowed
        if extra:
            unexpected.append((nid, sorted(extra)))
    if ax is None:
        missing.append(nid)
    elif "sorryAx" in ax:
        dirty.append(nid)
...
for nid, extra in unexpected:
    print("  forbidden %s: %s" % (nid, ", ".join(extra)))

if missing or unexpected:
    sys.exit("check_axioms: unresolved declarations or forbidden axioms")
...
unregistered_debt = [nid for nid in dirty if states[nid] != "DRAFT_SORRY"]
if unregistered_debt and "--emit" not in sys.argv:
    sys.exit("check_axioms: non-draft declarations depend on sorryAx: " +
             ", ".join(unregistered_debt))
sys.exit(0)
