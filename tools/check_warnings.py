#!/usr/bin/env python3
"""Build UCPlanar and verify its warnings against the frozen manifest.

Accepted diagnostics are the Lake Mathlib URL spelling notice and one Lean `declaration uses 'sorry'` diagnostic
...
def main() -> int:
    ...
    if build.returncode != 0:
        print(
            f"check_warnings: FAIL (lake build UCPlanar exited {build.returncode})",
            file=sys.stderr,
        )
        return 1
    ...
    for raw_line in build.stdout.splitlines():
        ...
        if line == "warning: manifest out of date: git url of dependency 'mathlib' changed; use `lake update mathlib` to update it":
            continue
